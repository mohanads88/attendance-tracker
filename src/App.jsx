...
import { Users, ScanLine, ClipboardList, Plus, Trash2, Download, Loader2, Check, X, AlertTriangle, Upload, Pencil, RotateCcw, CloudOff, RefreshCw, ChevronDown, ChevronUp } from "lucide-react";
...
const C = {
  ...
  blue:"#1d6fa4",blueSoft:"#e8f2fa",
};
const FONT=`'Tajawal', system-ui, sans-serif`;

function downscale(file,maxDim=1400,quality=0.82){
...
async function analyzePeriod(images,roster){
  ...
  // present: { id -> rawNameFromScreenshot }
  const present={};
  (data.present||[]).forEach((p)=>{if(p&&p.num) present[p.num]=p.name||"";});
  return{present,outOfList:data.outOfList||[],flags:data.flags||[]};
}

const EMPTY_PERIOD={images:[],present:{},outOfList:[],flags:[],analyzing:false,error:null,analyzed:false};
...
export default function App(){
  ...
  const resetMeeting=()=>{
    setP1(EMPTY_PERIOD);setP2(EMPTY_PERIOD);setOverrides({});
  };
  ...
  const merged=useMemo(()=>roster.map((m)=>{
    ...
    // evidence = raw name exactly as seen in the screenshot
    const rawP1=p1.present[m.id]||"";
    const rawP2=p2.present[m.id]||"";
    ...
    <div dir="rtl" style={{fontFamily:FONT,background:C.bg,minHeight:"100vh",color:C.ink}}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Tajawal:wght@400;500;700;800&display=swap');
        *{box-sizing:border-box}button{font-family:inherit;cursor:pointer}input{font-family:inherit}
        ...
        .row-hover:hover{background:#f0f7f3!important}
        ...
        <div style={{maxWidth:1080,margin:"0 auto",display:"flex",gap:2,padding:"0 12px"}}>
          {[["analyze","التحليل",ScanLine],["results","النتيجة",ClipboardList],["roster","قائمة المدعوين",Users]].map(([key,label,Icon])=>(
            <button key={key} onClick={()=>setTab(key)} style={{display:"flex",alignItems:"center",gap:8,background:"none",border:"none",padding:"14px 16px",fontSize:15,fontWeight:700,color:tab===key?C.green:C.muted,borderBottom:tab===key?`3px solid ${C.green}`:"3px solid transparent",marginBottom:-1}}>
              <Icon size={17}/>{label}
              {key==="results"&&hasResults&&<span style={{background:C.greenSoft,color:C.green,borderRadius:20,padding:"1px 9px",fontSize:12}}>{stats.present}</span>}
            </button>
          ))}
        </div>
      </nav>
      ...
      <main style={{maxWidth:1080,margin:"0 auto",padding:"22px 16px 60px"}}>
        ...
        {tab==="results"&&<ResultsTab merged={merged} stats={stats} outOfList={outOfList} flags={allFlags} setOverride={setOverride} p1={p1} p2={p2} setP1={setP1} setP2={setP2} roster={roster} onGoAnalyze={()=>setTab("analyze")}/>}
...
function AnalyzeTab({roster,p1,setP1,p2,setP2,onDone}){
  const hasAny=p1.analyzed||p2.analyzed;
  return(
    <div>
      <p style={{marginTop:0,color:C.muted,fontSize:14,lineHeight:1.7}}>
        ارفع لقطات قائمة المشاركين لكل فترة ثم اضغط «حلّل». يمكنك إعادة التحليل بعد إضافة لقطات جديدة أو حذف القديمة — النتيجة تتحدث في كل مرة.
      </p>
      ...
      {hasAny&&(
        <div style={{marginTop:20,textAlign:"center"}}>
          <button onClick={onDone} style={btnPrimary}>عرض النتيجة المدمجة ←</button>
        </div>
      )}
    </div>
  );
...
function PeriodCard({label,state,setState,roster}){
  ...
  const run=async()=>{
    ...
    try{
      const r=await analyzePeriod(state.images,roster);
      setState((s)=>({...s,...r,analyzing:false,analyzed:true}));
    }catch(e){
  ...
  const clearAll=()=>setState((s)=>({...s,images:[],present:{},outOfList:[],flags:[],analyzed:false,error:null}));
    ...
    <section style={card}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          {state.analyzed&&<span style={{background:C.greenSoft,color:C.green,borderRadius:20,padding:"3px 12px",fontSize:13,fontWeight:700}}>{count} مطابقة</span>}
          {state.images.length>0&&<button onClick={clearAll} title="مسح الكل" style={{...iconBtn,color:C.absent}}><Trash2 size={15}/></button>}
        </div>
      </div>

      {/* Drop zone */}
      <div onClick={()=>inputRef.current?.click()} onDragOver={(e)=>e.preventDefault()} onDrop={(e)=>{e.preventDefault();onFiles(e.dataTransfer.files);}}
        style={{border:`2px dashed ${C.line}`,borderRadius:14,padding:"18px 14px",textAlign:"center",cursor:"pointer",background:C.bg,marginBottom:state.images.length?12:0}}>
        <Upload size={20} color={C.green}/>
        <div style={{marginTop:6,fontSize:14,fontWeight:600}}>اسحب اللقطات هنا أو اضغط للرفع</div>
        <div style={{fontSize:12,color:C.muted,marginTop:2}}>PNG أو JPG — يمكن إضافة المزيد في أي وقت</div>
        <input ref={inputRef} type="file" accept="image/*" multiple hidden onChange={(e)=>onFiles(e.target.files)}/>
      </div>

      {/* Image thumbnails with individual remove */}
      {state.images.length>0&&(
        <div style={{display:"flex",flexWrap:"wrap",gap:8,marginBottom:12}}>
          {state.images.map((im,i)=>(
            <div key={i} style={{position:"relative"}}>
              <img src={im.dataUrl} alt="" style={{width:56,height:56,objectFit:"cover",borderRadius:8,border:`1px solid ${C.line}`,display:"block"}}/>
      ...
      {/* Error */}
      {state.error&&(
        <div style={{marginBottom:12,background:C.absentSoft,color:C.absent,padding:"10px 12px",borderRadius:10,fontSize:13}}>
          {state.error}
        </div>
      )}
      ...
      <button onClick={run} disabled={state.analyzing||state.images.length===0}
        style={{...btnPrimary,width:"100%",opacity:state.analyzing||!state.images.length?0.55:1,
          background:state.analyzed?C.blue:C.green}}>
        {state.analyzing
          ?<><Loader2 size={17} style={{animation:"spin 1s linear infinite"}}/> جارٍ التحليل…</>
          :state.analyzed
            ?<><RefreshCw size={17}/> إعادة التحليل</>
            :<><ScanLine size={17}/> حلّل الفترة</>}
      </button>

      {/* Flags from this period */}
      {state.flags.length>0&&(
        <div style={{marginTop:12,background:C.goldSoft,borderRadius:10,padding:"10px 12px",fontSize:13,color:"#6f4e12"}}>
          <b style={{display:"block",marginBottom:4}}>⚠️ نقاط للمراجعة:</b>
          <ul style={{margin:0,paddingInlineStart:18,lineHeight:1.8}}>{state.flags.map((f,i)=><li key={i}>{f}</li>)}</ul>
        </div>
      )}

      {/* Out of list from this period */}
      {state.outOfList.length>0&&(
        <div style={{marginTop:10,fontSize:13,color:C.muted}}>
          <b style={{color:C.gold}}>خارج القائمة ({state.outOfList.length}):</b>{" "}
          {state.outOfList.join("، ")}
        </div>
      )}
    </section>
  );
...
// ================= Results Tab =================
function ResultsTab({merged,stats,outOfList,flags,setOverride,p1,setP1,p2,setP2,roster,onGoAnalyze}){
  ...
  const exportExcel=()=>{
    const header=["م","الاسم","المنصب","الفترة الأولى","الفترة الثانية","الحالة","الاسم في اللقطة (الدليل)"];
    ...
    const pctRow=1+body.length+10;
    const pctCell=`B${pctRow}`;if(ws[pctCell])ws[pctCell].z="0.0%";
    ...
    <div>
      ...
      {flags.length>0&&(
        <div style={{background:C.goldSoft,border:"1px solid #ecd9a8",borderRadius:12,padding:"12px 14px",marginBottom:16}}>
          <div style={{display:"flex",alignItems:"center",gap:8,fontWeight:700,color:"#8a5d0e",marginBottom:6}}><AlertTriangle size={16}/> نقاط تحتاج مراجعتك — اضغط على ✓/✗ في الجدول للتصحيح</div>
          <ul style={{margin:0,paddingInlineStart:20,fontSize:13,color:"#6f4e12",lineHeight:1.8}}>{flags.map((f,i)=><li key={i}>{f}</li>)}</ul>
        </div>
      )}

      {noData&&(
        <div style={{...card,textAlign:"center",color:C.muted,padding:"40px 20px",marginBottom:16}}>
          لا توجد نتائج بعد. <button onClick={onGoAnalyze} style={{background:"none",border:"none",color:C.green,fontWeight:700,cursor:"pointer",fontSize:"inherit"}}>ارفع اللقطات وحلّل →</button>
        </div>
      )}
      ...
      <div style={{...card,padding:0,overflow:"hidden"}}>
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:14}}>
            ...
            <tbody>
              {filtered.map((m)=>(
                <tr key={m.id} className="row-hover" style={{background:m.present?C.presentSoft:C.absentSoft,borderBottom:`1px solid ${C.line}`,transition:"background 0.15s"}}>
                  <td style={{...td,textAlign:"center",color:C.muted,fontSize:12}}>{m.id}</td>
                  <td style={td}>
                    <div style={{fontWeight:700}}>{m.name}</div>
                    <div style={{fontSize:11,color:C.muted}}>{m.position}</div>
                  </td>
                  ...
                  <td style={{...td,textAlign:"center"}}>
                    <span style={{fontWeight:800,fontSize:13,color:m.present?C.present:C.absent}}>{m.present?"حضر":"لم يحضر"}</span>
                  </td>
      ...
      {outOfList.length>0&&(
        <div style={{...card,marginTop:16}}>
          <button onClick={()=>setShowOut(v=>!v)} style={{background:"none",border:"none",display:"flex",alignItems:"center",gap:8,fontWeight:800,fontSize:15,color:C.gold,cursor:"pointer",width:"100%",textAlign:"right"}}>
            حضور إضافي خارج القائمة ({outOfList.length})
            {showOut?<ChevronUp size={16}/>:<ChevronDown size={16}/>}
          </button>
          {showOut&&(
            <div style={{display:"flex",flexWrap:"wrap",gap:8,marginTop:12}}>
              {outOfList.map((n,i)=><span key={i} style={{background:C.goldSoft,border:"1px solid #ecd9a8",borderRadius:20,padding:"5px 12px",fontSize:13,color:"#6f4e12"}}>{n}</span>)}
            </div>
          )}
        </div>
      )}
...
function Toggle({on,onClick}){
  return(
    <button onClick={onClick} title={on?"حاضر — اضغط للتغيير":"غائب — اضغط للتغيير"}
      style={{width:30,height:30,borderRadius:8,border:`1.5px solid ${on?C.present:C.line}`,background:on?C.present:"#fff",color:on?"#fff":C.line,display:"grid",placeItems:"center",transition:"all 0.15s"}}>
...
function Stat({n,label,color,bg,active,onClick}){
  return(
    <div onClick={onClick} style={{background:bg,borderRadius:14,padding:"16px 18px",cursor:onClick?"pointer":"default",outline:active?`2px solid ${color}`:"none",transition:"outline 0.15s"}}>
      ...
      <div style={{...card,padding:0,overflow:"hidden"}}>
        <table style={{width:"100%",borderCollapse:"collapse",fontSize:14}}>
          <thead><tr style={{background:C.greenDark,color:"#fff"}}>
            <th style={{...th,width:42}}>م</th><th style={th}>الاسم</th><th style={th}>المنصب</th><th style={{...th,width:90}}></th>
          </tr></thead>
          <tbody>
            {roster.map((r)=>(
